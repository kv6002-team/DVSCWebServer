...
/**
 * A Builder for generating plain text.
 */
class TextBuilder implements Builder {
     ...
     * Make a TextBuilder.
     * 
     * @param RequestHandler $builderFn A request handler that returns plain
     *   text content.
     */
    ...
    /**
     * Allow the builder function to construct the page's content, then
     * construct a response from the return value.
     * 
    ...
    public function __invoke($request, ...$args) {
        ...
        return new Response($code, ["Content-Type" => "text/plain"], $content);
    }
    ...
    /**
     * A utility method to create a ContentTypeSelector that only supports plain
     * text.
     * 
     * @param RequestHandler $builderFn A request handler that returns plain
     *   text content.
